...
class UserService:
    ...
    async def create_user_service(self, user_data: UserCreate) -> APIResponse[UserResponse]:
        ...
        if existing_user:
            return APIResponse.error_response("Email already registered")

        # Validate role exists and get role object
        if user_data.role_name == "system admin" or user_data.role_name == "admin":
            return APIResponse.error_response(f"You cannot create an {user_data.role_name} user")

        role = await self.validate_role_exists(user_data.role_name)
        if not role:
            return APIResponse.error_response("Invalid role name")
        ...
        if user_data.organization_name:
            ...
            if not organization:
                return APIResponse.error_response("Invalid organization name")
        ...
        try:
            ...
            return APIResponse.success_response("User created successfully", UserResponse.model_validate(user))
    ...
    async def get_user_service(self, user_id: UUID) -> APIResponse[Optional[UserResponse]]:
        ...
        if user:
            return APIResponse.success_response("User fetched successfully", UserResponse.model_validate(user))
        return APIResponse.error_response("User not found")
    ...
    async def get_users_service(self, skip: int = 0, limit: int = 100) -> APIResponse[List[UserResponse]]:
        ...
        return APIResponse.success_response("Users fetched successfully", [UserResponse.model_validate(user) for user in users])
    ...
    async def update_user_service(self, user_id: UUID, update_data: UserUpdate, updated_by: Optional[UUID] = None) -> APIResponse[Optional[UserResponse]]:
        ...
        if user:
            return APIResponse.success_response("User updated successfully", UserResponse.model_validate(user))
        return APIResponse.error_response("User not found")
    ...
    async def delete_user_service(self, user_id: UUID, deleted_by: Optional[UUID] = None) -> APIResponse[str]:
        ...
        if response:
            return APIResponse.success_response("User deleted successfully")
        return APIResponse.error_response("Failed to delete user")
